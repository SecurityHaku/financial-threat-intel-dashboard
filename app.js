...
async function loadDashboardData() {
    ...
    try {
        ...
        if (topCves.length > 0) {
            try {
                const epssRes = await fetch(`/api/epss?cves=${topCves.join(',')}`).then(r => r.json());
                epssCacheMap = epssRes || {};
            } catch (e) {
...
async function searchCustomCve() {
    ...
    try {
        const res = await fetch(`/api/cve?id=${encodeURIComponent(query)}`).then(r => r.json());
        if (res.error) {
...
async function saveBookmarksToServer() {
    try {
        await fetch('/api/bookmarks', {
            ...
            body: JSON.stringify(globalBookmarks)
        });
        updateBadgesAndKPIs();
    } catch (e) {
...
async function searchCustomCveModal(cveId, bodyElem) {
    try {
        const res = await fetch(`/api/cve?id=${encodeURIComponent(cveId)}`).then(r => r.json());
        const nvdUrl = `https://nvd.nist.gov/vuln/detail/${cveId}`;
